{{--
    Root is a `<row>` so the `@desktop` sidebar sits beside the content rather
    than above it. `side_nav` is a fixed-width column on macOS; a row is what
    makes that a split, and a column would make it a header.
--}}
<native:row class="w-full h-full bg-slate-950">

    @desktop
        @include('native.partials.desktop-sidebar', ['active' => 'dashboard'])
    @enddesktop

    <native:column class="w-full h-full p-8 gap-6">
